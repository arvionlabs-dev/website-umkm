<template>
  <div class="flex flex-col min-h-screen items-center pt-4 md:pt-8 justify-center">
    <div class="flex flex-col lg:flex-row gap-4 md:mt-0 md:gap-8 pt-20 lg:pt-0 px-4 md:px-8 lg:px-16 w-full max-w-7xl mx-auto">
      <div class="flex-1 w-full relative order-2 lg:order-1 overflow-hidden rounded-lg md:rounded-2xl bg-gray-200">
        <nuxt-img 
          v-if="currentUMKM.foto && currentUMKM.foto.length > 0 && !heroImageError"
          class="object-cover w-full absolute"
          :src="currentUMKM.foto[0]"
          :alt="currentUMKM.nama"
          @error="handleHeroImageError"
          loading="lazy"
        />
        <div v-else class="w-full h-full min-h-[400px] lg:min-h-full flex flex-col items-center justify-center gap-2">
          <Icon name="mdi:store" class="text-6xl text-gray-400" />
          <p class="text-gray-500 text-sm">{{ currentUMKM.nama }}</p>
        </div>
      </div>
      
      <div class="flex-1 flex flex-col gap-3 order-1 lg:order-2">
        <h1 class="text-3xl md:text-4xl font-bold w-full">
          {{ currentUMKM.nama }}
        </h1>
        ...
        <div v-if="currentUMKM.kategori && currentUMKM.kategori.length > 0" class="flex flex-wrap gap-2">
          <span
            v-for="kategori in currentUMKM.kategori"
            :key="kategori"
            class="text-sm px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full font-medium"
          >
            {{ kategori }}
          </span>
        </div>
        ...
        <div v-if="kontakArray.length > 0" class="flex flex-col gap-2 mt-2">
          ...
          <div class="flex flex-col gap-2">
            <a 
              v-for="(contact, index) in kontakArray" 
              :key="index"
  ...
  <!-- Location Section -->
  <div class="flex flex-col items-center justify-center w-full px-4 md:px-16 my-16 max-w-7xl mx-auto">
    <p class="mb-4 text-base w-full md:text-lg lg:text-xl flex gap-2 items-start md:items-center">
      <span class="text-2xl shrink-0 items-center justify-center flex flex-col">
        <Icon name="material-symbols:location-on-outline" class="text-yellow-600" />
      </span>
    ...
    <Map 
      v-if="currentUMKM.gmapIframeSrc"
      :height-in-px="300" 
      :show-google-map-link="true" 
      :gmap-src="currentUMKM.gmapSrc"
  ...
  <div v-if="currentUMKM.foto && currentUMKM.foto.length > 0" class="px-4 md:px-8 lg:px-16 w-full pb-12 md:pb-16 max-w-7xl mx-auto">
...
// Check if current UMKM is favorited
...
// Track click for recommendations when component mounts
onMounted(() => {
  if (currentUMKM.value.slug) {
